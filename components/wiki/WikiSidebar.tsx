              ...
              <div className="mb-2">
                <div className="flex items-center justify-between px-2 py-1.5">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground uppercase tracking-wider transition-colors"
                  >
                    {collapsed['_sessions'] ? <ChevronRight className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                    {typeIcons.SESSION_RECAP}
                    <span>{t('sidebar.sessions')}</span>
                    <span className="text-muted-foreground text-[10px]">{recaps.length}</span>
                  </button>
                  {!isLocked && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={onCreateSessionClick}>
              ...
              <div className="mb-1">
                <div className="flex items-center justify-between px-2 py-1.5">
                  ...
                  >
                    ...
                    <span className="text-muted-foreground text-[10px]">{wikiEntries.length}</span>
                  </button>
                  {!isLocked && (
                    <div className="flex items-center gap-0.5">
                      <Tooltip>
                        ...
                        <TooltipContent>{t('sidebar.newWikiPage')}</TooltipContent>
                      </Tooltip>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={onUpdateWikiClick}>
                            <RefreshCw className="w-3.5 h-3.5" />
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent>{t('sidebar.updateWikiAI')}</TooltipContent>
                      </Tooltip>
                    </div>
                  )}
                </div>
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="absolute inset-y-0 right-0 w-px bg-transparent group-hover/resize:bg-primary/50 transition-colors duration-150" />
        </div>
      </aside>
    </>
  )
